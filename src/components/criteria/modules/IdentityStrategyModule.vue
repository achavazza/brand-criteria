...
const rawIndicators = computed(() => tm('identityStrategy.indicators') as unknown as Array<{ title: string; question: string; left: string; right: string }>)
...
async function captureStepper() {
  ...
  const key = 'is:capture'
  if (captures.isCaptured(key)) { captures.remove(key); return }
  try {
    const vals = Object.values(values.value).join('-')
    captures.add('identity-strategy', `Sliders [${vals}]`, await captureElement(stepperRef.value), key)
  } catch { /* ignore */ }
}
</script>
...
<template>
  <div class="strategy-module">
    <div class="module-header">
      ...
      <button class="capture-btn mono" :class="{ captured: captures.isCaptured('is:capture') }" @click="captureStepper">Capture</button>
    </div>
    ...
    <div ref="stepperRef" class="stepper-grid" :class="{ captured: captures.isCaptured('is:capture') }">
