...
const rawIndicators = computed(() => tm('semanticRelevance.indicators') as unknown as Array<{ title: string; question: string; left: string; right: string }>)
...
async function captureStepper() {
  ...
  const key = 'sr:capture'
  if (captures.isCaptured(key)) { captures.remove(key); return }
  try {
    const vals = Object.values(values.value).join('-')
    captures.add('semantic-relevance', `Sliders [${vals}]`, await captureElement(stepperRef.value), key)
  } catch { /* ignore */ }
}
</script>

<template>
  <div class="semantic-module">
    <div class="module-header">
      <span class="module-label mono">{{ t('modules.formMeaning') }}</span>
      <button class="capture-btn mono" :class="{ captured: captures.isCaptured('sr:capture') }" @click="captureStepper">Capture</button>
    </div>
    ...
    <div ref="stepperRef" class="stepper-grid" :class="{ captured: captures.isCaptured('sr:capture') }">
...
.large-logo { max-width: 75%; max-height: 85%; object-fit: contain; }
